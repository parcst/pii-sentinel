...
export default function JiraBanner() {
  const jiraStatus = useScanStore((s) => s.jiraStatus);
  const jiraValid = useScanStore((s) => s.jiraValid);
  const jiraValidationError = useScanStore((s) => s.jiraValidationError);
  const setJiraSetupOpen = useScanStore((s) => s.setJiraSetupOpen);

  if (!jiraStatus) return null;

  // Configured + validated successfully
  if (jiraStatus.configured && jiraValid === true) {
    return (
      <div className="flex items-center justify-between rounded-lg bg-gray-800/60 border border-gray-700/50 px-3 py-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-gray-300 truncate">
            Jira linked ({jiraStatus.projectKeys?.join(', ')})
          </span>
        </div>
        <button
  ...
  if (jiraStatus.configured && jiraValid === false) {
    return (
      <div className="rounded-lg bg-red-950/30 border border-red-800/40 p-3 space-y-1.5">
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-red-400 shrink-0" />
          <span className="text-xs font-medium text-red-200">Jira connection failed</span>
        </div>
        <p className="text-xs text-red-300/70 leading-relaxed">
          {jiraValidationError || 'Could not connect to the configured Jira instance.'}
        </p>
        <button
          onClick={() => setJiraSetupOpen(true)}
  ...
  if (jiraStatus.configured && jiraValid === null) {
    return (
      <div className="flex items-center gap-2 rounded-lg bg-gray-800/60 border border-gray-700/50 px-3 py-2">
        ...
        <span className="text-xs text-gray-400">Checking Jira connection...</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg bg-blue-950/40 border border-blue-800/40 p-3 space-y-2">
      <p className="text-xs text-blue-200/90 leading-relaxed">
        Link Jira to create Bug tickets for PII columns on two project boards simultaneously.
      </p>
      <button
        onClick={() => setJiraSetupOpen(true)}
